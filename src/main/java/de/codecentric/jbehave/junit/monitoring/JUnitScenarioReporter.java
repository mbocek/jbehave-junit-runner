    ...
    public void afterStory(boolean isGivenStory) {
        ...
        if (isGivenStory) {
            ...
            if (!failedSteps.contains(currentStoryDescription)) {
                ...
                // also failed.
                if (!anyScenarioFailedInCurrentStory) {
                    notifier.fireTestFinished(currentStoryDescription);
                } else {
                    notifier.fireTestFailure(new Failure(currentStoryDescription,
                        new RuntimeException( "story failed!")));
                }
    ...
    public void beforeScenario(String title) {
        ...
        if (!givenStoryContext) {
            ...
            if (children.size() > examples.size()) {
                ...
                ArrayList<Description> steps = new ArrayList<>(currentScenario.getChildren());
