...
public class JUnitDescriptionGeneratorTest {
    ...
    public void shouldCopeWithSeeminglyDuplicateSteps() throws Exception {
        when(scenario.getSteps()).thenReturn(Arrays.asList(new String[]{"Given Step1", "Given Step1"}));
        generateScenarioDescription();
    ...
    public void shouldGenerateDescriptionForExampleTablesOnScenario() {
        ...
        for (Description exampleDescription : description.getChildren()) {
            assertThat(exampleDescription.getChildren(),
                hasItem(Matchers.<Description>hasProperty("displayName", startsWith("Given Step1"))));
    ...
    public void shouldGenerateChildrenForComposedSteps() {
        ...
        when(steps.listCandidates()).thenReturn(
            Arrays.asList(new StepCandidate[]{stepCandidate, composedStep1, composedStep2}));
        generator = new JUnitDescriptionGenerator(Arrays.asList(new CandidateSteps[]{steps}), configuration);
        ...
        assertThat(composedStep.getChildren(),
            everyItem(Matchers.<Description>hasProperty("displayName", startsWith("compositeStep"))));
    ...
    public void shouldSkipExampleTablesForParameterizedGivenStories() {
        ...
        addStepToScenario();
        when(givenStories.getPaths()).thenReturn(Arrays.asList("/some/path/to/GivenStory.story#{0}"));
